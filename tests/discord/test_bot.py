...
def generate_game_info(
    player_turn: Optional[str], seconds_remaining: Optional[int], state: GameState, match_id
) -> GameInfo:
...
class TestBot:
    @pytest.mark.parametrize(
        "match_id,game_info,sent_message,always_send,expected",
        [
            (
                "message_sent-start",
            ...
            (
                "message_already_sent_lt_24h-req_turn",
                generate_game_info(
                    "player", 60 * 60 * 10, GameState.IN_PROGRESS, "message_already_sent_lt_24h-req_turn"
                ),
                Message("player", 15),
                True,
                ":rotating_light:",
    ...
    def test_decide_send_message(
        self, match_id: str, game_info: GameInfo, sent_message: Message, always_send: bool, expected: Optional[str]
    ):
